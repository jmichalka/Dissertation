...
catalytic properties.\citep{Yu:2012by, Kim:2013mi, Jenness:2013oj, Han:2015qr}  In
...
electronic structure which is in constrast to non-metal molecular species.
Unlike other chemical species which tend to only share valence electron density
with nearest bonded neighbors, metals are able to freely share their valence
electrons throughout the material.  This ``sea of electrons'' can be described
using an extension of molecular orbital theory, where instead of only have a
few bonding and anti-bonding molecular orbitals at various energy levels, there
are enough atomic orbitals from the numerous metal atoms to form {\em bands} of
bonding and antibonding orbitals which are labeled as ``valence'' and
``conduction'' bands. In metals, there is essentially no energy barrier for an
electron to move from the top of the valence band to the conduction band which
is one of the reasons metals tend to have high electrical and thermal
conductivies, as compared to ``insulators'' which typically have large energy
gaps between the HOMO and the LUMO orbitals.
...
sized small spheres. The highest packing density can be achieved with either an ABC
layer stacking which results in a face-centered cubic (FCC) crystal structure
or an ABA layer stacking which results in the hexagonal close-packed (HCP)
...
\begin{figure}[p!]
  ...
  \caption{Depiction of ABC and ABA layer stacking for FCC and HCP crystal
structures. The lowest layer of atoms are ``A'', while the middle, gray, atoms
represent the ``B'' layer. The atoms in the third layer can sit either directly
above a hollow site in the middle layer and an atom in the bottom layer leading
to the ABA stacking, or sit directly above two hollow sites leading to ABC
stacking.}
  \label{fig:packing}
\end{figure}
...
size of metal surfaces leads to one of the most commonly observed defects being
...
between plateaus. Due to their undercoordination, these edge atoms will
interact differently with adsorbates, potentially making those sites 
catalytically active.

There are additional surface atoms that are under-coordinated on metal surfaces
and are often labeled as kink atoms, describing an atom that is at the vertex
of two intersecting edges, or adatoms, free atoms sitting on an otherwise flat
surface. These atoms, owing to their undercoordination are all promising
candidates for enhanced catalytic activity and industrial catalysts attempt to
increase the density of these sites. Nanoparticles and nanospheres owing to
their curvature have a large number of edge, kink, and adatoms making them of
significant industrial interest.  However, the size of nanoparticles makes
simulating them costly and instead by ``slicing'' along certain Miller indices,
much smaller surfaces can be constructed that still contain a large number of
edge and kink atoms.
...
at examining various bimetallic species, including heterogenous alloys,
core-shell nanoparticles, and near-surface alloys so as to design more
effective catalysts.  Figure \ref{fig:bimetallic} shows a number of ideal
examples of these types of systems. While synthesizing these systems can be
...
costs\citep{Li:0hl, Zhao:0qf} provides a strong impetus for the
...
industrial processes make use of high-index or roughenend nanoparticles taking
...
of edge and kink atoms creates additional binding possiblities on the surface
...
independently, defined as diffusion, or 2) in a concerted fashion, defined as
step-wandering. While it is helpful to divide these as two distinct modes, they
are in many ways coupled.  Independent adatom movement is statistically more
likely to be observed as it only requires one surface atom to be ejected from a
stable edge or terrace or upwards from a surface.  Since the strength of
metallic bonding is proportional to the number of nearest neighbors, once an
adatom is created and is seated on the surface, there is often only a minimal
energy barrier for the adatom to continue diffusing around the surface.
...
events. Understanding the dynamics of step-edges is important because
reconstruction events tend to involve the appearance or disappearance of
step-edges as their presence tends to raise the surface energy of a system
...
electronic bands, the introduction of surfaces break that symmetry. This results in the rehybridization of
...
low-index FCC and HCP surfaces. Depending on the energy levels of the adsorbate orbitals
...
As a specific example, when CO was placed in the atop
position, the CO $5\sigma$ (HOMO) orbital only interacted with 3 of Pt's frontier
orbitals, the $6s$, $5p_z$, and
...
allowed for favorable dipole-dipole interactions to exist between the atop
...
can be observed with XPS (X-Ray Photoelectron Spectroscopy), LEED (Low Energy
Electron Diffraction), and other experimental techniques. The preferred binding
sites are dependent on coverage, since the presence and amount of bound
adsorbates affects the energy levels of the surface and in particular the
energies at the different binding sites.

Thus, at distinct coverages, different patterns are observed. For example,
...
also be able to overcome any kinetic barriers that would trap the surface in a
...
depending on the specific system and conditions and has been estimated to occur
...
identify the reconstructions, they are often unable to identify the actual
...
reconstructions.\citep{Williams:1994aa, Jeong:1999ev, Eren:2016qt} 
...
system the step-edges doubled, but upon removal of the \ce{CO} the original
...
significant research has been devoted to increasing the surface area to bulk
